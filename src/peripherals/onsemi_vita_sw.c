...
int onsemi_vita_init( onsemi_vita_t *pContext, char szName[], Xuint32 uBaseAddr_SPI, Xuint32 uBaseAddr_CAM )
{
   ...
   if ( uCoreID == ONSEMI_VITA_SPI_CORE_ID_VAL )
   {
      ...
      {
         //printf( "[onsemi_vita_init] Detected Compatible Version %d.%d.%d of ONSEMI_VITA_SPI core\n\r", ((uCoreVersion&0xFF000000)>>24), ((uCoreVersion&0x00FF0000)>>16), (uCoreVersion&0x0000FFFF) );
      }
      else
      {
         //printf( "[onsemi_vita_init] Detected Incompatible Version %d.%d.%d of ONSEMI_VITA_SPI core\n\r", ((uCoreVersion&0xFF000000)>>24), ((uCoreVersion&0x00FF0000)>>16), (uCoreVersion&0x0000FFFF) );
         return 0;
      }
   }
   else
   {
      //printf( "[onsemi_vita_init] Failed to detect ONSEMI_VITA_SPI core\n\r" );
      return 0;
   }
   ...
   if ( uCoreID == ONSEMI_VITA_CAM_CORE_ID_VAL )
   {
      ...
      {
         //printf( "[onsemi_vita_init] Detected Compatible Version %d.%d.%d of ONSEMI_VITA_CAM core\n\r", ((uCoreVersion&0xFF000000)>>24), ((uCoreVersion&0x00FF0000)>>16), (uCoreVersion&0x0000FFFF) );
      }
      else
      {
         //printf( "[onsemi_vita_init] Detected Incompatible Version %d.%d.%d of ONSEMI_VITA_CAM core\n\r", ((uCoreVersion&0xFF000000)>>24), ((uCoreVersion&0x00FF0000)>>16), (uCoreVersion&0x0000FFFF) );
         return 0;
      }
   }
   else
   {
      //printf( "[onsemi_vita_init] Failed to detect ONSEMI_VITA_CAM core\n\r" );
      return 0;
   }
...
int onsemi_vita_spi_read( onsemi_vita_t *pContext, Xuint16 uAddr, Xuint16 *pData )
{
   ...
   if ( !timeout )
   {
	   printf( "[onsemi_vita_spi_read ] Timed out waiting for !TXFIFO_FULL\n\r" );
	   return 0;
   }
   ...
   if ( !timeout )
   {
	   printf( "[onsemi_vita_spi_read ] Timed out waiting for !RXFIFO_EMPTY\n\r" );
	   *pData = 0x0000;
	   return 0;
   }
   ...
   //printf( "[onsemi_vita_spi_read ] Response = 0x%08X\n\r", uResponse );
...
int onsemi_vita_spi_write( onsemi_vita_t *pContext, Xuint16 uAddr, Xuint16 uData )
{
   ...
   {
	  uStatus = onsemi_vita_spi_reg_read(pContext, ONSEMI_VITA_SPI_STATUS_REG);
	  //printf( "[onsemi_vita_spi_write] Status   = 0x%08X\n\r", uStatus );
   }
   while ( (uStatus & ONSEMI_VITA_SPI_TXFIFO_FULL_BIT) && !(--timeout) );

   if ( !timeout )
   {
	   printf( "[onsemi_vita_spi_write] Timed out waiting for !TXFIFO_FULL\n\r" );
	   return 0;
   }

   // Send Request
   uRequest = (ONSEMI_VITA_SPI_WRITE_BIT) | (((Xuint32)uAddr) << 16) | ((Xuint16)uData);
   //printf( "[onsemi_vita_spi_write] Request  = 0x%08X\n\r", uRequest );
   onsemi_vita_spi_reg_write(pContext, ONSEMI_VITA_SPI_TXFIFO_REG, uRequest);
...
int onsemi_vita_spi_nop( onsemi_vita_t *pContext )
{
   ...
   {
	  uStatus = onsemi_vita_spi_reg_read(pContext, ONSEMI_VITA_SPI_STATUS_REG);
	  //printf( "[onsemi_vita_spi_nop  ] Status   = 0x%08X\n\r", uStatus );
   }
   while ( (uStatus & ONSEMI_VITA_SPI_TXFIFO_FULL_BIT) && !(--timeout) );

   if ( !timeout )
   {
	   printf( "[onsemi_vita_spi_nop  ] Timed out waiting for !TXFIFO_FULL\n\r" );
	   return 0;
   }

   // Send Request
   uRequest = (ONSEMI_VITA_SPI_NOP_BIT) | 0x00000000;
   //printf( "[onsemi_vita_spi_nop  ] Request  = 0x%08X\n\r", uRequest );
   onsemi_vita_spi_reg_write(pContext, ONSEMI_VITA_SPI_TXFIFO_REG, uRequest);
  ...
  int i;
   ...
   if ( (initID == SENSOR_INIT_SEQ00) || (initID == SENSOR_INIT_ENABLE) )
   {
      // Configure Sync Generator
      {
         ...
         if ( bVerbose )
         {
        	 printf( "VITA SYNCGEN - Setting Video Timing\n\r" );
        	 printf( "\tHSYNC Timing     = hav=%04d, hfp,=%02d, hsw=%02d (hsp=%d), hbp=%03d (x4)\n\r",
               h_active, h_fporch, h_syncwidth, h_syncpol, h_bporch);
        	 printf( "\tVSYNC Timing     = hav=%04d, hfp,=%02d, hsw=%02d (hsp=%d), hbp=%03d (x4)\n\r",
      ...
      if ( bVerbose ) printf( "VITA REMAPPER - Configuring for image lines in normal mode\n\r" );
      ...
      if ( bVerbose ) printf("VITA SPI Sequence 0 - Releasing RESET_N pin\n\r" );
      ...
      if ( bVerbose ) printf( "\tVITA_SPI[0x%04X] => 0x%04X\n\r", uAddr, uData );
	  if ( bVerbose )
	  {
         switch ( uData )
		 {
      ...
      if ( uData != 0x5614 )
      {
         if ( bVerbose ) printf( "\tERROR: Absent or unsupported VITA sensor !!!\n\r" );
         return 0;
      }
   }
   ...
   if ( (initID == SENSOR_INIT_SEQ02) || (initID == SENSOR_INIT_ENABLE) )
   {
      ...
      if ( bVerbose ) printf("VITA SPI Sequence 2 - Verify PLL Lock Indicator\n\r" );
      uAddr = 24;
      ...
      if ( !timeout )
      {
		  if ( bVerbose ) printf( "\tERROR: Timed Out while waiting for PLL lock to assert !!!\n\r" );
         return 0;
      }
   }
   ...
   if ( (initID == SENSOR_INIT_SEQ06) || (initID == SENSOR_INIT_ENABLE) )
   {
	         ...
	         if ( bVerbose ) printf( "VITA ISERDES - Status = 0x%08X\n\r", uStatus );

	         uStatus = onsemi_vita_cam_reg_read( pContext, ONSEMI_VITA_CAM_ISERDES_STATUS_REG );
	         if ( bVerbose ) printf( "VITA ISERDES - Status = 0x%08X\n\r", uStatus );

	         if ( bVerbose ) printf( "VITA ISERDES - Waiting for CLK_RDY to assert\n\r");
	         uStatus = onsemi_vita_cam_reg_read( pContext, ONSEMI_VITA_CAM_ISERDES_STATUS_REG );
	         if ( bVerbose ) printf( "VITA ISERDES - Status = 0x%08X\n\r", uStatus );
	         timeout = 9;
	         ...
	         if ( !timeout )
	         {
	            if ( bVerbose ) printf( "\tTimed Out !!!\n\r" );
	            return 0;
	         }

	         if ( bVerbose ) printf( "VITA ISERDES - Align Start\n\r" );
	         onsemi_vita_cam_reg_write( pContext, ONSEMI_VITA_CAM_ISERDES_CONTROL_REG, ONSEMI_VITA_CAM_ISERDES_ALIGN_START_BIT );

	         if ( bVerbose ) printf( "VITA ISERDES - Waiting for ALIGN_BUSY to assert\n\r");
	         uStatus = onsemi_vita_cam_reg_read( pContext, ONSEMI_VITA_CAM_ISERDES_STATUS_REG );
	         if ( bVerbose ) printf( "VITA ISERDES - Status = 0x%08X\n\r", uStatus );
	         timeout = 9;
	         ...
	         if ( bVerbose ) printf( "VITA ISERDES - Waiting for ALIGN_BUSY to de-assert\n\r");
	         uStatus = onsemi_vita_cam_reg_read( pContext, ONSEMI_VITA_CAM_ISERDES_STATUS_REG );
	         if ( bVerbose ) printf( "VITA ISERDES - Status = 0x%08X\n\r", uStatus );
	         timeout = 9;
      ...
      if ( bVerbose ) printf( "VITA ISERDES - Enabling FIFO enable\n\r" );
      onsemi_vita_cam_reg_write( pContext, ONSEMI_VITA_CAM_ISERDES_CONTROL_REG, ONSEMI_VITA_CAM_ISERDES_FIFO_ENABLE_BIT );
      if ( bVerbose ) printf( "VITA DECODER - Enabling Sync Channel Decoder\n\r" );
      ...
      if ( bVerbose ) printf( "VITA DECODER - Control = 0x%08X\n\r", uControl );

      timeout = 10;

      do {
      uStatus = onsemi_vita_cam_reg_read( pContext, ONSEMI_VITA_CAM_CRC_STATUS_REG );
      if ( bVerbose ) printf( "VITA CRC - Status = 0x%08X\n\r", uStatus );
   ...
   if ( (initID == SENSOR_INIT_SEQ07) || (initID == SENSOR_INIT_DISABLE) )
   {
      if ( bVerbose )
      {
         printf("VITA SPI Sequence 7 - Disable Sequencer\n\r" );
   ...
   if ( (initID == SENSOR_INIT_SEQ08) || (initID == SENSOR_INIT_DISABLE) )
   {
      if ( bVerbose )
      {
         printf("VITA SPI Sequence 8 - Soft Power-Down\n\r" );
   ...
   if ( (initID == SENSOR_INIT_SEQ09) || (initID == SENSOR_INIT_DISABLE) )
   {
      if ( bVerbose )
      {
         printf("VITA SPI Sequence 9 - Disable Clock Management - Part 2\n\r" );
   ...
   if ( (initID == SENSOR_INIT_SEQ10) || (initID == SENSOR_INIT_DISABLE) )
   {
      if ( bVerbose )
      {
         printf("VITA SPI Sequence 10 - Disable Clock Management - Part 1\n\r" );
...
//vspi write 192 0x0000
   if ( bVerbose ) printf( "VITA 1080P60 - Disable Sequencer\n\r");
...
//vspi rmw 192 0x0040 0x0040
   if ( bVerbose ) printf( "VITA 1080P60 - Adjust line spacing in VITA\n\r");
...
//vreg write 0x5C 0x00000CE4
   if ( bVerbose ) printf( "VITA 1080P60 - Tolerate 6 lines of jitter (required for programmable exposure)\n\r");
   onsemi_vita_cam_reg_write( pContext, 0x5C, 0x00000CE4 );
   if ( bVerbose ) printf( "\tVITA_REG[0x%04X] <= 0x%08X\n\r", 0x5C, 0x00000CE4 );
...
//vreg write 0x64 0x0094802C
   if ( bVerbose ) printf( "VITA 1080P60 - Adjust line spacing in sync generator\n\r");
...
//vspi rmw   194 0x0000 0x0000
   if ( bVerbose ) printf( "VITA 1080P60 - Adjust frame spacing in VITA\n\r");
   ...
   //vreg write 0x6C 0x00248005
      if ( bVerbose ) printf( "VITA 1080P60 - Adjust frame spacing in sync generator\n\r");
...
//vspi write 258 0x0474
   if ( bVerbose ) printf( "VITA 1080P60 - Crop ROI0 from 1920x1200 to 1920x1080\n\r");
...
//vspi write 160 0x0010
   if ( bVerbose ) printf( "VITA 1080P60 - Disable auto-exposure\n\r");
...
//trig internal 60 10
   if ( bVerbose ) printf( "VITA 1080P60 - Enable trig generator\n\r");
   {
...
//vspi rmw 192 0x0011 0x0011
   if ( bVerbose ) printf( "VITA 1080P60 - Enable sequencer\n\r");
   ...
   usleep(100); // 100 usec
#else
  if ( bVerbose ) printf( "VITA 1080P60 - Exposure related settings\n\r");
...
int onsemi_vita_set_analog_gain( onsemi_vita_t *pContext, Xuint32 uAnalogGain, int bVerbose )
{
   ...
   if ( bVerbose )
   {
      printf( "VITA-2000 SPI Sequency - Analog Gain\n\r" );
...
int onsemi_vita_set_digital_gain( onsemi_vita_t *pContext, Xuint32 uDigitalGain, int bVerbose )
{
   ...
   if ( bVerbose )
   {
	  printf( "VITA-2000 SPI Sequency - Digital Gain\n\r" );
	  onsemi_vita_spi_display_sequence( pContext, seqData, seqLen );
   }
   onsemi_vita_spi_write_sequence( pContext, seqData, seqLen );
...
int onsemi_vita_set_exposure_time( onsemi_vita_t *pContext, Xuint32 exposureTime, int bVerbose )
{
   ...
   vitaTrigGenDefaultFreq = (((1920+88+44+148)*(1080+4+5+36))>>2) - 2;
   if ( bVerbose ) printf( "\tTrigger = internal (%d fps, duty cycle = %d \%, period = %d cycles)...\r\n", trigFramesPerSec, trigDutyCycle, vitaTrigGenDefaultFreq+2 );

   onsemi_vita_cam_reg_write( pContext, ONSEMI_VITA_CAM_TRIGGEN_DEFAULT_FREQ_REG, vitaTrigGenDefaultFreq );
   if ( bVerbose ) printf( "\t0x%08X <= 0x%08X\n\r", ONSEMI_VITA_CAM_TRIGGEN_DEFAULT_FREQ_REG, vitaTrigGenDefaultFreq );
   //vitaTrigGenTrig0High   = (vitaTrigGenDefaultFreq * trigDutyCycle)/100; // positive polarity
   vitaTrigGenTrig0High   = (vitaTrigGenDefaultFreq * (100-trigDutyCycle))/100; // negative polarity
   vitaTrigGenTrig0Low    = 1;
   onsemi_vita_cam_reg_write( pContext, ONSEMI_VITA_CAM_TRIGGEN_TRIG0_HIGH_REG  , vitaTrigGenTrig0High   );
