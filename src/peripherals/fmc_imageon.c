...
int fmc_imageon_init( fmc_imageon_t *pContext, char szName[], fmc_iic_t *pIIC )
{
   ...
   //printf( "[fmc_imageon_init] PCA9534[0x%02X] <= 0x%02X\n\r", reg_addr, reg_data );
...
{   
   ...
   //printf( "Release reset from I2C mux ...\n\r" );
   // Modify appropriate GPIO bits
...
void fmc_imageon_iic_mux( fmc_imageon_t *pContext, Xuint32 MuxSelect )
{
   ...
   //printf("[fmc_imageov_iic_mux] I2C_MUX=%d\n\r", MuxSelect );
                              ...
                              Xuint8 ConfigData[][2], Xuint32 ConfigLength )
{
   ...
   {
      //printf( "[fmc_imageon_iic_config2] IIC Write - Device = 0x%02X, Address = 0x%02X, Data = 0x%02X\n\r", ChipAddress<<1, ConfigData[i][0], ConfigData[i][1] );
                              ...
                              Xuint8 ConfigData[][3], Xuint32 ConfigLength )
{
   ...
   for ( i = 0; i < ConfigLength; i++ )
   {
      //printf( "[fmc_imageon_iic_config3] IIC Write - Device = 0x%02X, Address = 0x%02X, Data = 0x%02X\n\r", ConfigData[i][0]<<1, ConfigData[i][1], ConfigData[i][2] );
...
void fmc_imageon_vclk_init( fmc_imageon_t *pContext )
{
   ...
   for ( i = 0; i < MAX_IIC_CDCE913; i++ )
   {
      ...
      //printf("[fmc_imageon_config_vclk] CDCE913[0x%02X] <= 0x%02X\n\r",(0x80 | iic_cdce913[i][0]), (iic_cdce913[i][1]));
...
int fmc_imageon_hdmii_init( fmc_imageon_t *pContext, Xuint32 Enable, Xuint32 edidInit, Xuint8 pEdid[256] )
{
   ...
   if ( !Enable )
   {
      //printf("\tDe-asserting HotPlugDetect\n\r");
   ...
   else
   {
      ...
      //printf("\tDe-asserting Reset\n\r");
      fmc_imageon_hdmii_set_rst( pContext, 0 );
      fmc_imageon_wait_usec(1000000);


#if 0
      //printf( "I2C Initializing - Reset\n\r" );
      ...
      //printf("I2C Initializing - Mapping\n\r");
      fmc_imageon_iic_mux( pContext, FMC_IMAGEON_I2C_SELECT_HDMI_IN );
...
int fmc_imageon_hdmii_set_hpd( fmc_imageon_t *pContext, Xuint32 HotPlugStatus )
{
   ...
   reg_addr = 0x01;
   num_bytes = pContext->pIIC->fpIicRead( pContext->pIIC, FMC_IMAGEON_IO_EXP_ADDR, reg_addr, &reg_data, 1); 
   //printf( "[fmc_imageon_hdmii_set_hpd] PCA9534[0x%02X] <= 0x%02X\n\r", reg_addr, reg_data );
   if ( HotPlugStatus )
...
int fmc_imageon_hdmii_set_rst( fmc_imageon_t *pContext, Xuint32 Reset )
{
   ...
   reg_addr = 0x01;
   num_bytes = pContext->pIIC->fpIicRead( pContext->pIIC, FMC_IMAGEON_IO_EXP_ADDR, reg_addr, &reg_data, 1); 
   //printf( "[fmc_imageon_hdmii_set_rst] PCA9534[0x%02X] <= 0x%02X\n\r", reg_addr, reg_data );
   if ( Reset )
...
int fmc_imageon_hdmii_get_int( fmc_imageon_t *pContext, Xuint32 *pIntStatus )
{
   ...
   reg_addr = 0x00;
   num_bytes = pContext->pIIC->fpIicRead( pContext->pIIC, FMC_IMAGEON_IO_EXP_ADDR, reg_addr, &reg_data, 1); 
   //printf( "[fmc_imageon_hdmii_get_int] PCA9534[0x%02X] <= 0x%02X\n\r", reg_addr, reg_data );
...
int fmc_imageon_hdmii_get_lock( fmc_imageon_t *pContext )
{
   ...
   //printf( "[fmc_imageon_hdmii_get_lock] ADV7611-HDMI4[0x%02X] <= 0x%02X\n\r", reg_addr, reg_0x07);
...
int fmc_imageon_hdmio_init( fmc_imageon_t *pContext, Xuint32 Enable, fmc_imageon_video_timing_t *pTiming, Xuint32 WaitForHPD )
{
   ...
   else
   {
	  ...
	  // Adjust HDMI/DVI mode based on specified video timing info
	  {
		 ...
		 for ( i = 0; i < IIC_HDMI_OUT_CONFIG_LEN; i++ )
		 {
		    if ( iic_hdmi_out_config[i][1] == 0xAF )
			{
				if ( pTiming->IsHDMI )
				{
                   if ( pContext->bVerbose ) printf( "\tSetting I2C config sequence for HDMI Mode\n\r" );
                   iic_hdmi_out_config[i][2] = 0x06; // Set HDMI Mode
                                                     //    R0xAF[  1] = HDMI/DVI Mode Select = 1 (HDMI Mode)
				}
				else
				{
                   if ( pContext->bVerbose ) printf( "\tSetting I2C config sequence for DVI Mode\n\r" );
      ...
      if ( pContext->bVerbose )
	  {
         ...
         for ( i = 0; i < 6; i++ )
         {
            printf("\tembedded_sync_config[%d] = 0x%02X, 0x%02X, 0x%02X\n\r",
               i,
      ...
      if (num_bytes < 1)
      {
         //printf("\t\tERROR occurred during I2C transaction !\n\r");
      }
      ...
      if ( WaitForHPD )
      {
         ...
         while (((i2c_data & 0xC0) != 0xC0) && (--timeout))
         {
            ...
            if (num_bytes < 1)
            {
               printf( "[fmc_imageon_hdmio_init] ERROR occurred during I2C transaction !\n\r");
               timeout = 0;
               return 0;
            }
            //printf("\t0x%02X[0x%02X] => 0x%02X\n\r", (i2c_dev << 1), i2c_addr, i2c_data);
         ...
         if (!timeout)
         {
            printf( "[fmc_imageon_hdmio_init] ERROR - Timeout waiting for internal HotPlugDetect !\n\r");
            return 0;
         }
      } // if ( WaitForHPD )
   }
...
int fmc_imageon_hdmio_set_pd( fmc_imageon_t *pContext, Xuint32 PowerDown )
{
   ...
   reg_addr = 0x01;
   num_bytes = pContext->pIIC->fpIicRead( pContext->pIIC, FMC_IMAGEON_IO_EXP_ADDR, reg_addr, &reg_data, 1); 
   //printf( "[fmc_imageon_hdmio_set_pd] PCA9534[0x%02X] <= 0x%02X\n\r", reg_addr, reg_data );
   if ( PowerDown )
...
int fmc_imageon_hdmio_get_hpd( fmc_imageon_t *pContext, Xuint32 *pHotPlugDetect )
{
   ...
   reg_addr = 0x00;
   num_bytes = pContext->pIIC->fpIicRead( pContext->pIIC, FMC_IMAGEON_IO_EXP_ADDR, reg_addr, &reg_data, 1); 
   //printf( "[fmc_imageon_hdmio_get_hpd] PCA9534[0x%02X] <= 0x%02X\n\r", reg_addr, reg_data );
